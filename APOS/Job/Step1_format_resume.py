...
# remove department,type
def remove_department_type(json_data):
    for i in xrange(len(json_data)):
        jobs = json_data[i]["workExperienceList"]
        for job_id in xrange(len(jobs)):
            ...
            if job is not None:
                ...
                job.pop("department")
                job.pop("industry")
                json_data[i]["workExperienceList"][job_id] = job
    return json_data
...
if __name__ == "__main__":
    ...
    trn_file = open(util.raw_data_prefix + "practice2.json", "w")
    for person in new_trn_json_data:
        write_json_dict(person, trn_file)
    trn_file.close()
    # remove department and type since they are usually empty
    trn_file = open(util.raw_data_prefix + "resume.json", "w")
    new_trn_json_data = remove_department_type(new_trn_json_data)
    for person in new_trn_json_data:
        write_json_dict(person, trn_file)
    trn_file.close()
