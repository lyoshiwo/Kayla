...
f_m = get_manual_feature(train)
print f_m.shape

from step8_mlp import get_cluster_or_w2v_feature

f_c_w = get_cluster_or_w2v_feature(data, True, True)
f_c_w = np.array(f_c_w)

features = np.concatenate((f_m, f_c_w), axis=1)
print features.shape

from step7_tree import test_rf
from sklearn.cross_validation import train_test_split
